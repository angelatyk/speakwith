...
"""
    ...
    try:
        ...
        for q_num_str, content in matches:
            ...
            if 1 <= q_num <= len(HISTORICAL_FIGURE_QUESTIONS):
                ...
                is_question_repetition = (
                    content_lower.startswith(question_lower[:50]) or  # Starts with question
                    (question_lower in content_lower and len(content) < len(question_text) + 100)  # Contains question but short
                )
        ...
        if len(result['answers']) < len(HISTORICAL_FIGURE_QUESTIONS) * 0.5:  # Less than 50% parsed
            result['answers'] = {}
            ...
            for line in lines:
                line = line.strip()
                if not line:
                    if current_answer:  # Empty line might separate Q&A, but continue current answer
                        current_answer += "\n"
                    continue
...
def generate_elevenlabs_voice_summary(answers: Dict) -> str:
    ...
    for question in voice_questions:
        if question in answers and answers[question]:
            ...
            if answer and answer.lower() not in ['n/a', 'not available', 'unknown', 'uncertain']:
...
def get_or_create_historical_figure(person_name: str) -> Dict:
    ...
    if existing:
        ...
        if 'elevenlabs' not in existing or not existing.get('elevenlabs'):
            ...
            elevenlabs_summary = generate_elevenlabs_voice_summary(existing.get('answers', {}))
...
def list_historical_figures():
    ...
    collection = db[HISTORICAL_FIGURES_COLLECTION]
    figures = list(collection.find({}, {'person_name': 1, 'person_name_lower': 1, 'created_at': 1}))
...
def generate_character_context(figure_data: Dict) -> str:
    ...
    if 'What is their full name and any known aliases or nicknames?' in answers:
        context_sections.append(f"Name: {answers['What is their full name and any known aliases or nicknames?']}")
    
    if 'What time period did they live in (specific years and era)?' in answers:
        context_sections.append(f"Time Period: {answers['What time period did they live in (specific years and era)?']}")
    ...
    if personality_info:
        context_sections.append(f"Personality: {' '.join(personality_info[:3])}")  # Limit to avoid token bloat
    ...
    if 'What was their profession, occupation, or role did they hold?' in answers:
        context_sections.append(f"Profession: {answers['What was their profession, occupation, or role did they hold?']}")
    
    if 'What are they most famous or known for?' in answers:
        context_sections.append(f"Known For: {answers['What are they most famous or known for?']}")
    
    # Quirks and mannerisms
    if 'What were their personal quirks, habits, or idiosyncrasies?' in answers:
        context_sections.append(f"Quirks: {answers['What were their personal quirks, habits, or idiosyncrasies?']}")
    
    # Philosophical views
    if 'What were their philosophical views on life, death, and purpose?' in answers:
        context_sections.append(f"Philosophy: {answers['What were their philosophical views on life, death, and purpose?']}")
    
    return "\n".join(context_sections)

def simulate_conversation(person_name: str, user_message: str, conversation_history: List[Dict] = None) -> str:
    """
        ...
        if results['documents'] and results['documents'][0]:
            for doc, metadata, distance in zip(
                ...
                formatted_results.append({
                    ...
                    'answer': doc,
                    'relevance_score': 1 - distance if 'distances' in results else None
                })
...
def conversation(person_name):
    ...
    try:
        ...
        # Generate response
        response = simulate_conversation(person_name, user_message, conversation_history)
